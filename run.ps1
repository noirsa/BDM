...
if ($HasGpu) {
    Write-Host "NVIDIA GPU detected. Starting full stack with GPU support and 3 Spark workers..." -ForegroundColor Cyan
    $ComposeFiles = @("-f", "docker-compose.yml", "-f", "docker-compose.gpu.yml")
}
else {
    Write-Host "No NVIDIA GPU detected. Starting full stack without GPU and with 3 Spark workers..." -ForegroundColor Yellow
    $ComposeFiles = @("-f", "docker-compose.yml")
}

docker compose @ComposeFiles up --build -d --scale spark-worker=3
